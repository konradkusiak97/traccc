...
/// Function used for creating the 2D measurement objects out of the clusters in each module
///
/// The output is a measurement container with 1 measurement corresponding to 1 cluster
///
/// @param[in] globalIndex          The index for the current thread
/// @param[in] clusters_view        Container storing the cells for every cluster
/// @param[in] cells_view           The cells for each module
/// @param[out] measurements_view   Container storing the created measurements for each module
///
TRACCC_HOST_DEVICE
