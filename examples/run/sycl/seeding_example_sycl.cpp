...
int seq_run(const std::string& detector_file, const std::string& hits_dir,
    ...
    for (unsigned int event = 0; event < events; ++event) {
        ...
        auto sa_sycl_result = sa_sycl(spacepoints_per_event);
        auto& seeds_sycl = sa_sycl_result.second;
        n_seeds_sycl += seeds_sycl.get_headers()[0];
        ...
        if (!skip_cpu) {
            ...
            for (auto seed : seeds.get_items()[0]) {
                if (std::find(seeds_sycl.get_items()[0].begin(),
                              seeds_sycl.get_items()[0].begin() +
                                  seeds_sycl.get_headers()[0],
                              seed) != seeds_sycl.get_items()[0].begin() +
                                           seeds_sycl.get_headers()[0]) {
                    n_match++;
                }
            }
